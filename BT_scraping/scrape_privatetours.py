#!/usr/bin/env python3
"""
Scrape all private tour URLs and extract data into private_tours_frontend.json
Reads URLs from private_tour_urls.json (generated by get_privatetour_urls.py)
"""
...
def scrape_all_privatetours():
    """Scrape all private tours from the JSON URL list"""

    # Load the URLs from JSON file
    try:
        with open('private_tour_urls.json', 'r') as f:
        ...
        print("❌ Error: private_tour_urls.json not found!")
        print("   Run get_privatetour_urls.py first to discover URLs")
        return []
    except Exception as e:
        print(f"❌ Error loading URLs from JSON: {e}")
        return []

    print(f"🚀 Processing {len(urls)} private tour URLs...")
    print("=" * 70)
    ...
    for i, url in enumerate(urls, 1):
        ...
        if i % 10 == 0:
            ...
            with open(f'private_tours_progress_{i}.json', 'w') as f:
    ...
    timestamped_filename = f'private_tours_frontend_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'

    # Write main file using atomic write
    main_success, main_error = atomic_write_json(final_data, 'private_tours_frontend.json')
    if not main_success:
    ...
    if main_success or backup_success:
        if main_success:
            print(f"📁 Saved main file: private_tours_frontend.json")
        if backup_success:
    ...
    progress_files = glob.glob('private_tours_progress_*.json')
    for progress_file in progress_files:
        safe_remove_file(progress_file)

    # Remove backup files older than 7 days
    current_time = time.time()
    for backup_file in glob.glob('private_tours_frontend_backup_*.json'):
        try:
    ...
    for timestamped_file in glob.glob('private_tours_frontend_2*.json'):
    ...
    print(f"   📁 Results saved to: private_tours_frontend.json")
