...
def main():
    try:
        ...
        # Step 2.5: Merge CSV data to create enhanced JSON
        set_status('running', 90, 'Merging CSV availability data...', total_urls, total_urls)
        log_message("Step 2.5: Merging CSV data with availability information...")

        success, output = run_command('python3 merge_csv_data.py', 'CSV data merge')
        if not success:
            log_message(f"⚠️  Warning: CSV merge had issues: {output}")
            # Don't fail the entire process if merge fails
        else:
            log_message("✅ CSV data merge completed - enhanced JSON created")
