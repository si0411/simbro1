...
def extract_dates_from_json_string(date_string):
    ...
    Also handles year rollover: "10 Dec - 1 Jan 2027" means Dec 2026 to Jan 2027
    ...
    try:
        ...
        # Construct start date string with end year initially
        start_date_str = f"{start_part} {year}"
        start_date = parse_json_date(start_date_str)

        # Handle year rollover: if start month > end month, start is previous year
        # e.g., "10 Dec - 1 Jan 2027" means Dec 2026 to Jan 2027
        if start_date and end_date and start_date.month > end_date.month:
            start_date_str = f"{start_part} {int(year) - 1}"
            start_date = parse_json_date(start_date_str)
